...
- Amount (built-in) site convention: [How Empower Amount is calculated](skills/empower-custom-fields/references/amount-empower.md)
- First lesson: [Assay and dilution records](skills/empower-custom-fields/references/assay-single-area.md)
- Known impurity (workbook vs external std): [Percentage known impurity](skills/empower-custom-fields/references/percentage-known-impurity.md)
...
- 2026-09-23: User taught Empower Amount equation and site conventions (Dilution and Purity(%) default; Value = std weight; Sample weight Sample/Keyboard; Standard_Potency Component in Amount table; SD Component / TD Sample; prefer Amount then modify). Decoded Assay_Single_Area / Dilution_Factor against Amount. Compared workbook `Percentage_Known_Impurity` to user external impurity-std equation; labels S1/U1; Peak Labels K/U/M; Path 1/Path 2 drafts. Added `amount-empower.md` and `percentage-known-impurity.md`; revised assay lesson; wired CF Studio sync/keywords. No Empower execution recorded.
