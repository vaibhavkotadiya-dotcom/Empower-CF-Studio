...
Match the intended calculation and preparation workflow, not only keywords. Reuse the recorded formula only when its assumptions fit. If syntax, units, molecular-weight direction, reference selection or required settings are missing, ask the questions needed to resolve them and label any interim output a draft. Never silently repair missing multiplication signs. Do not generalize one site's defaults or use built-in Amount and additional dilution factors without checking for duplicate correction.
