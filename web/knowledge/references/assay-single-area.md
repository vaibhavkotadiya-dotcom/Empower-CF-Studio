...
Revision: 4, updated 2026-09-14 after importing `Custom_Field_F1.xlsx`.
Sources: [supplied conversation](source-conversation.txt), user formula dated Aug 19 and subsequent user answers labeled Today; [workbook transcript](source-custom-field-f1.md), supplied 2026-09-14. The conversation transcript does not establish the calendar date of Today.
Status: user-provided formulas and partially confirmed conventions. The workbook adds configuration evidence and related fields, but no Empower execution result, version/build, or complete calculation settings. The previous assistant's word "validated" is not validation evidence.
...
|---|---|---|
| Amount | Built-in field used as input | Exact calibration configuration, units, built-in Dilution and SampleWeight inputs unknown; earlier assistant's explanation is not independently verified here |
| Dilution_Factor | Separate preparation factor; SD is standard, TD is sample/test | Formula user-confirmed in follow-up; detailed input meanings and settings remain unknown |
| Standard_Potency | Percent, e.g. 99.8 | User-confirmed; no default provided |
...
- Still unknown: the exact mapping of each field to a final flask volume or transferred aliquot, unused-input defaults, numeric entry format, field scope, data type and configuration. The operator positions alone are not confirmation of the physical preparation steps.
